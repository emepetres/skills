...
These are not obvious from the files, and one of them broke the marketplace once:
...
- The plugin entry in `marketplace.json` uses `source: "./"` — the repo itself is the single bundled plugin, not one plugin per skill.
- A marketplace plugin entry needs only `name`, `source`, and `description` (plus optional `category` and `homepage`). Keep `version`, `author`, `license`, and `keywords` out of it — Claude Code reads those from the plugin's own `plugin.json`, and duplicating them there just creates two things to bump.
